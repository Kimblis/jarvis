...
const orchestrateCleaning = (state: typeof GameStateAnnotation.State) => {
  const { organizations, flaggedRows, processedOrganizations, analysis } =
    state;
...
const analyzeData = async (state: typeof GameStateAnnotation.State) => {
  const { organizations, validationErrors } = state;
...
const cleanData = async (state: typeof GameStateAnnotation.State) => {
  const { organizations, analysis, flaggedRows, actionsPerformed } = state;
  ...
  return new Command({
    update: {
      ...
      messages: [
        ...state.messages,
        ...messages,
        new AIMessage({ content: JSON.stringify(cleanData) }),
      ],
...
export async function POST(req: NextRequest) {
  try {
    ...
    const graph = new StateGraph(GameStateAnnotation)
      ...
      .addEdge("analyze", "orchestrateCleaning")
      .compile();
